...
void moveShip(void) {
  ...
  if (shipState.state == SHIP_STATE_FLYING) {
        ...
        if ((ly == landingBlock[e_num].y) && (lx == landingBlock[e_num].x)) {
          ...
          case E_START:
            ...
            if (((sa <= (56 * 16)) || (sa >= (968 * 16))) &&
              ...
              Lsx = sx; // set next start position here when landed on fuel /
                        // start
              Lsy = sy;
            }
            break;
    ...
    if (num_of_collisions > 5 && shipState.state != SHIP_STATE_LANDED &&
        (trainer == FALSE)) { // SHIP IS EXPLOADING
    ...
    if (shipState.state == SHIP_STATE_FLYING && ShipFuel > 0) {
      shipState.image =
          (shipThrust == 0 ? SHIP_IMAGE_NO_THRUST : SHIP_IMAGE_THRUST);
    } else {
