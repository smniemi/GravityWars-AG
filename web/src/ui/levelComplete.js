...
export class LevelCompleteScreen {
    ...
    show(stats) {
        ...
        if (highEl)
            highEl.textContent = storedHigh > 0 ? storedHigh.toLocaleString() : '---';
        if (scoreEl)
    ...
    showNameEntryOverlay(defaultName, onSubmit) {
        ...
        const input = overlay.querySelector('#pilot-name-input');
        const submitBtn = overlay.querySelector('#submit-name-btn');
        if (defaultName) {
