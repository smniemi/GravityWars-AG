export class LevelCompleteScreen {
    ...
    public show(stats: { level: number, time: number, bestTime: number, globalBest: number }) {
        this.isVisible = true;
        this.element.classList.remove('hidden');

        const title = this.element.querySelector('#lc-title');
        if (title) title.textContent = `Completed Level ${stats.level} !!!`;

        const time = this.element.querySelector('#lc-time');
        if (time) time.textContent = `Time: ${stats.time.toFixed(1)} sec`;

        const best = this.element.querySelector('#lc-best');
        if (best) best.textContent = `Your best time: ${stats.bestTime.toFixed(1)} sec`;

        const global = this.element.querySelector('#lc-global');
        if (global) global.textContent = `Global high score: ${stats.globalBest > 0 ? stats.globalBest.toFixed(1) : '-'} sec`;
    }
